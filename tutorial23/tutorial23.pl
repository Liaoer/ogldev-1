...
has [
    qw( pGameCamera VBO_vertex_size VBO_tex_offset VBO_normal_offset VBO pEffect pMesh pQuad pShadowMapTech shadowMapFBO )
] => ( is => 'rw' );
...
sub Init {
    ...
    $self->pShadowMapTech( ShadowMapTechnique->new );

    if ( !$self->pShadowMapTech->Init ) {
        printf( "Error initializing the shadow map technique\n" );
        return;
    }

    $self->pShadowMapTech->Enable;
...
sub ShadowMapPass {
    ...
    $self->pShadowMapTech->SetWVP( $p->GetWVPTrans );
    $self->pMesh->Render;
...
sub RenderPass {
    ...
    $self->pShadowMapTech->SetTextureUnit( 0 );
    $self->shadowMapFBO->BindForReading( GL_TEXTURE0 );
...
sub main {
    ...
    if ( !GLUTBackendCreateWindow( WINDOW_WIDTH, WINDOW_HEIGHT, 0, "Tutorial 23" ) ) {
        return 1;
    }

    my $pApp = tutorial23->new;
